...
### 2. 전체 접근 전략 (요약)
----
- 불필요한 배경 및 플레이트 외곽 영향을 제거하기 위해 ROI 기준으로 crop한 뒤 처리했다.
...
- 각 셀은 하나의 웰 ROI로 처리
- `grid_debug.png`를 생성하여 분할 상태를 시각적으로 검증할 수 있도록 함
- 이유
...
- 이를 통해 과제 요구사항인 “내부 경계 제외”를 만족시켰다.

#### 3.3 조명 보정 (Illumination Flatten)
- 웰 내부 조명 편차로 인해 단순 threshold는 불안정
- 큰 sigma의 Gaussian blur로 배경을 근사한 뒤 정규화: `norm = image / background`
- 콜로니와 배경의 대비를 안정적으로 확보

#### 3.4 콜로니 Blob 분할
- 내부 영역(inner_mask)에서 밝기 분포 추출
- 대비(sigma)에 따라 adaptive percentile threshold 선택
- morphological open/close로 노이즈 제거
- connected components로 후보 blob 생성
- **seed 위치와의 거리 + 면적 기반 점수**로 최적 blob 선택
...
#### 4.2 중앙 점에 찍히는 밀집점 문제
- 초기 구현시 밀집점 계산을 픽셀 값 기준으로 했을 때, 웰 안의 작은 노이즈 점(중앙 근처의 작은 점) 으로 보라 격자가 찍히는 현상이 발생함
- 개선:
  - **중앙 노이즈 억제**: 밀집점 탐색 마스크에서 웰 중심부를 제외하여 중앙 패턴/광학 잡점이 최대값으로 선택되는 현상을 줄임 (final_mask)
  - **콜로니 영역 제한**: spots 기반 밀집점을 seed로 colony blob을 추출한 뒤, blob이 검출되면 blob 내부에서만 밀집점을 재계산하여 노이즈 점으로의 오검출을 방지함
...
- 실제 실험 이미지에서 발생할 수 있는 조명 변화, 약한 콜로니, 분할 실패 케이스에 대해서도 결과 누락 없이 후처리 가능한 형태로 구성하였다.
